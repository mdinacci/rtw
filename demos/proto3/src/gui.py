...
from pandac.PandaModules import TextNode, Point3, TransparencyAttrib
...
class BallSelectionScreen(Screen):
    ...
    def __init__(self, screenMgr):
        ...
        self._ball0 = GOM.createEntity(photon_ball_params)
        self._ball1 = GOM.createEntity(shark_ball_params)
        self._ball2 = GOM.createEntity(avg_joe_ball_params)
        self._ball3 = GOM.createEntity(turtle_king_ball_params)
...
class ScreenManager(object):
    ...
    def displayScreen(self, name, *params):
        if self.currentScreen is not None:
            self.previousScreen = self.currentScreen.name
        ...
        if name == "main":
            ...
            self.currentScreen = ExitRequestScreen(self)
        elif name == "options":
            self.currentScreen = OptionsMenuScreen(self)
        elif name == "profile":
            self.currentScreen = ProfileMenuScreen(self)
        elif name == "pause":
            self.currentScreen = PauseMenuScreen(self)
        elif name == "new-profile":
            self.currentScreen = NewProfileMenuScreen(self)
        elif name == "rename-profile":
            self.currentScreen = RenameProfileMenuScreen(self, params[0])
        elif name == "ball-selection":
            self.currentScreen = BallSelectionScreen(self)
        
        return self.currentScreen

    def displayPreviousScreen(self):
        self.displayScreen(self.previousScreen)
...
class VideoOptions(DirectFrame):
    ...
    normal_resolutions = ["640x480", "800x600","1024x768", "1280x1024", 
                          "1400x1050", "1600x1200"]
